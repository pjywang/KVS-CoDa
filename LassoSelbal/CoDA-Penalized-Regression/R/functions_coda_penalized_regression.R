...
#    proportions, intensities, ...), matrix of dimension n by k (with no zeros).
#    **specified colnames of X would be desired.
...
# r: fixed parameter (friction parameter) used for the update step.
# tol: tolerance for the difference explained deviance in two consecutive steps.
# tol2: tolerance to fulfill the constraint sum(beta[1: ])=0
...
#  library(MASS)  #only if ginv function is needed (not in our case)
  ...
  n  <-  nrow(X);
  z <- log(X)      # log transformation Z=log(X)
  ...
  c <- c(0,rep(1, p))
  c <- c/sqrt(normSqr(c)) # normSqr: squared norm.. 
  c <- as.matrix(c)       # becomes a column matrix (vector)
  p_c <- c%*%t(c)         # Projection matrix to the constraint set; cc^t
  
  # p_cginv <- ginv(p_c);  #use this only if necessary
  # To apply more complex constraints, we have to compute the generalized inverse!
  p_c_cginv <- p_c; #p_c%*%p_cginv; #this product in our case is = p_c
  # generalized inverse = original (Moore-Penrose pseudoinverse..)
  
  # (Centering!) z transformation for improvement of optimization
 ...
 # No warm start..!
  ...
  # null deviance.. need to get the "explained deviance"!!
  # ability of prediction using only the intercept (the lower the better)
  nulldev <- glm(y~1, family=binomial())[[10]]


 #initialization parameters

  k <- 1            # k: will count the number of iteration
  epsilon <- 0.1
  beta_ant <- beta_ini    # storage for b^(k-1)
  beta <- beta_ant
  y_ant <- beta_ant
  y_k <- y_ant
  d_k <- y_ant
  
  # If the change in explained deviance is lower then tol, it halts.
  # I don't know whether this is really reasonable for stopping rule..?
  dev_explained_ant <- 0
  ...
  start_iter = Sys.time();
    while((abs(epsilon) > tol) & (k <= maxiter)){
        ...
        k0 <- 0          # k0: count number of line search
      t_k <- 10
      condition <- 0.1
...
coda_logistic_elasticNet <- function(y,X,lambda, alpha=0.5, maxiter=600, maxiter2=50, r=10,
                                   tol=1.e-4, tol2=1.e-6, printTime=NULL){
...
# printTime: value to print computing time (works any value)
  ...
  while((abs(epsilon) > tol) & (k <= maxiter)){
    ...
    while ((condition > 0) & (k0 <= maxiter2)){
      ...
      # Soft thresholding:
      # zproxi: soft-thres(y^(k-1) - t_k * grad_g)
  ...
  # Projects beta-coordinates with > tol2
  # The following considerably reduces computing time of matrix multiplications..
  # but I wonder that we already have "zproj" variable.. why do this?
  indx <- which(abs(zproxi)>tol2)
  if (abs(zproxi[1]) > 0) indx <- indx[-1]
  c0 <- rep(1,(length(indx)))
  c0 <- c0/sqrt(normSqr(c0))
...
# Stable computation of log(1+exp)
log1plusexp <- function(x){
  if (x < -60) {return(0)}
  else if (x<40) {return(log(1+exp(x)))}
  else {return(x)}
}
...
g <- function(x,Z,Y,n){
  ...
  res <- res-sum(sum(sapply(aux,log1plusexp)))
  res <-  as.vector((-res)/n)
  return(res)
}
...
lambdaRange_codalasso  <-  function(y,X,lambdaSeq=seq(0,1,0.01),
                                    findlambda=FALSE, verbose=TRUE){
  if (verbose) print(c("lambda", "num.selected", "prop.explained.dev"))
  
  desired_lambda = c()
  desired_models = list()
  
  for (lambda in lambdaSeq){
    results  <-  coda_logistic_lasso(y,X,lambda 
                                     # maxiter = 100
                                     );
    numVarAct = as.integer(sum(abs(results$betas[2:length(results$betas)]) > 0))
    if (verbose) cat(sprintf("%.4f  %.0f  %.4f\n", lambda, numVarAct, results[[6]]))
    
    # 특정 numVar를 뽑는 lambda를 찾고 싶은 경우의 문법
    if (is.numeric(findlambda)){
      if (numVarAct %in% findlambda){
        desired_lambda = c(desired_lambda, lambda) 
        desired_models = append(desired_models, list(results)) # must be covered by list( * )
      }
    }
  }
  
  if (length(desired_lambda) == 0) {
   cat("Try finer lambdaSeq")
  }
  return(list("lamb" = desired_lambda,
